...
function onNewPlayer (data) {
  // Create a new player
  var newPlayer = new Player(data.x, data.y, data.angle, data.ver, data.health)
  newPlayer.id = this.id
  console.log('New player ' + newPlayer.id + ' uses: ' + newPlayer.getVer())

  // Broadcast new player to connected socket clients
  this.broadcast.emit('new player', {id: newPlayer.id, x: newPlayer.getX(), y: newPlayer.getY(), angle: newPlayer.getAngle(), ver: newPlayer.getVer(), health: newPlayer.getHealth()})

  // Send existing players to the new player
  var i, existingPlayer
  for (i = 0; i < players.length; i++) {
    existingPlayer = players[i]
    this.emit('new player', {id: existingPlayer.id, x: existingPlayer.getX(), y: existingPlayer.getY(), angle: existingPlayer.getAngle(), ver: existingPlayer.getVer(), health: existingPlayer.getHealth(), num_of_players: num_of_players})
