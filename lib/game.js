...
function onNewPlayer (data) {
  // Create a new player
  var newPlayer = new Player(data.x, data.y, data.angle, data.ver, data.health, data.team)
  newPlayer.id = this.id
  console.log('New player ' + newPlayer.id + ' uses: ' + newPlayer.getVer())

  // Broadcast new player to connected socket clients
  this.broadcast.emit('new player', {id: newPlayer.id, x: newPlayer.getX(), y: newPlayer.getY(), angle: newPlayer.getAngle(), ver: newPlayer.getVer(), health: newPlayer.getHealth(), team: newPlayer.getTeam()})

  // Send existing players to the new player
  var i, existingPlayer
  for (i = 0; i < players.length; i++) {
    existingPlayer = players[i]
    this.emit('new player', {id: existingPlayer.id, x: existingPlayer.getX(), y: existingPlayer.getY(), angle: existingPlayer.getAngle(), ver: existingPlayer.getVer(), health: existingPlayer.getHealth(), team: existingPlayer.getTeam(), num_of_players: num_of_players})
...
function onMovePlayer (data) {
  ...
  this.broadcast.emit('move player', {id: movePlayer.id, x: movePlayer.getX(), y: movePlayer.getY(), angle: movePlayer.getAngle(), ver: movePlayer.getVer(), health: movePlayer.getHealth()})
}
...
function onTakeDamage (data) {
  ...
  // Broadcast updated position to connected socket clients
  this.broadcast.emit('take damage', {id: playerDamaged.id, x: playerDamaged.getX(), y: playerDamaged.getY(), angle: playerDamaged.getAngle(), ver: playerDamaged.getVer(), health: playerDamaged.getHealth(), laser: data.laser})
}
